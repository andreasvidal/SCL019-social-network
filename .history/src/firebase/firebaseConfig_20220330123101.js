...
import { getFirestore, collection, addDoc, getDocs, onSnapshot, query, orderBy, updateDoc, deleteDoc, Timestamp } from 'https://www.gstatic.com/firebasejs/9.6.7/firebase-firestore.js';
...
export const readDataPost = async() => {
    const q = query(collection(db, "cities"), orderBy("date", "desc"));
    const unsubscribe = onSnapshot(q, (querySnapshot) => {
        const CommentBox = [];
        querySnapshot.forEach((doc) => {
            CommentBox.push(doc.data().name);
        });
        console.log("Current cities in CA: ", citi CommentBox.join(", "));
    });
}
...
export const likePost = async(id, userLike) => {
    ...
    const docSnap = await getDocs(likeRef); //estamos trayendo un post especifico con getDoc
