...
export const Post = () => {
    ...
    const newContainer = dataPost.querySelector('#containerPostAdd')
    const newAllPost = async() => {
        ...
        onSnapshot(collection(db, "post"), (querySnapshot) => {
            ...
            querySnapshot.forEach(doc => {
                ...
                html += `
                <div> 
                <h3 class="titlePost">${docPost.titulo}</h3>
                <textarea class="commentDone" readonly>${docPost.comentario}</textarea>
                </div>
                `
            })
