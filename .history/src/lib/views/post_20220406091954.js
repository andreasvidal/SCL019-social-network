...
export const printComments = (docPost, id) => {
    ...
    const updatePost = containerEmpty.querySelectorAll(".btnEdit");
    updatePost.forEach(docPost) => {
        docPost.addEventListener("click", () => {
            const btnValue = docPost.value; //llamamos al elemento especifico que queremos modificar
            const editDocReview = containerPostvoid.querySelector(`.commentDone-${valueButton}`);
        })
    }
