...
export const printComments = (docPost, id) => {
    ...
    const html = `<div id="containerPost">
        ...
        <textarea class="commentDone" id="commentDone-${btnValue}" readonly>${docPost.textArea}</textarea>
    ...
    updatePost.forEach((docPost) => {
        docPost.addEventListener("click", async() => {
            const btnValue = docPost.dataset; //llamamos al elemento especifico que queremos modificar
            ...
            const saveValue = containerPostvoid.querySelector(`#saveBtn-${docPost.value}-save`, ); //llamamos al boton para guardar cambios
